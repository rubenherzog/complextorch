...
def test_registry_matches_direct_pird_pdgc_and_reuses_hop_tensors():
    ...
    innovations = __import__("complextorch").var_to_innovations_state_space(system)
...
def test_confidence_api_covers_rates_and_high_order_from_one_resampling_ensemble():
    ...
    result = measure_confidence_intervals(
        ...
        measures=(
            "rates.o_information",
            "rates.mutual_information",
            "rates.transfer_entropy",
            "pird.redundant",
            "pdgc.synergistic",
            "spectral_pird.delta",
            "spectral_pdgc.delta",
...
def test_confidence_api_all_compatible_does_not_require_oir_for_univariate_var():
    ...
    assert result.n_valid >= 2
    assert all("o_information" not in name for name in result.intervals)
...
def test_registry_preserves_leading_var_batch_axis():
    ...
    assert tree["rates"]["o_information"].shape == (3,)
