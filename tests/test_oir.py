...
def test_spectral_oir_integrates_to_direct_temporal_oir():
    system = _dynamic_three()
    frequency = _grid(n=1025)
    spectral = spectral_o_information_rate(system, frequency)
    temporal = o_information_rate(system)
    integrated = integrate_spectral_rate(spectral, frequency)
...
def test_batched_oir_and_spectral_oir_match_explicit_system_loop():
    coefficients = torch.tensor(
        [
            [
