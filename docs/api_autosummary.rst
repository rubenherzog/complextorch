...
This internal page exists only to generate the per-object autodoc pages used by
:doc:`api`. The public API index uses explicit document links so that every
function and class name is reliably clickable on Read the Docs.
