...
def _unidirectional(dtype=torch.float64):
    ...
    coefficients = torch.tensor([[[0.55, 0.00], [0.42, 0.35]]], dtype=dtype)
...
def test_iid_independent_has_zero_information_and_predictive_rates():
    ...
    values = (
        gaussian_mutual_information_rate(system, [0], [1]),
        gaussian_transfer_entropy_rate(system, [0], [1]),
        gaussian_transfer_entropy_rate(system, [1], [0]),
        gaussian_instantaneous_information_rate(system, [0], [1]),
    )
    for value in values:
        torch.testing.assert_close(
            value, torch.zeros_like(value), atol=1e-11, rtol=0.0
        )
...
def test_unidirectional_var_has_forward_but_not_reverse_transfer_entropy():
    ...
    torch.testing.assert_close(
        _as_scalar(integrate_spectral_rate(forward_spectrum, frequency)),
        forward,
        rtol=2e-7,
        atol=2e-9,
    )
    torch.testing.assert_close(
        _as_scalar(integrate_spectral_rate(forward_gc_spectrum, frequency)),
        forward_gc,
        rtol=2e-7,
        atol=2e-9,
    )
...
def test_independent_dynamic_blocks_have_zero_cross_block_information_rate():
    ...
    for value in (
        ...
        torch.testing.assert_close(
            value, torch.zeros_like(value), atol=2e-10, rtol=0.0
        )
...
def test_integrated_innovations_spectrum_recovers_zero_lag_covariance():
    ...
    # Independent Lyapunov solve via vectorisation, not ComplexTorch dynamics.
    transition = system.transition[0]
    observation = system.observation[0]
    gain = system.gain[0]
    innovation = system.innovation_covariance[0]
    q = gain @ innovation @ gain.transpose(-1, -2)
    r = transition.shape[0]
    p = torch.linalg.solve(
        torch.eye(r * r, dtype=transition.dtype)
        - torch.kron(transition, transition),
        q.reshape(-1),
    ).reshape(r, r)
    expected = observation @ p @ observation.transpose(-1, -2) + innovation
    torch.testing.assert_close(integrated[0], expected, rtol=3e-6, atol=3e-7)
...
def test_information_rates_are_invariant_to_invertible_within_block_transforms():
    ...
    torch.testing.assert_close(
        gaussian_mutual_information_rate(transformed, [0, 1], [2, 3]),
        gaussian_mutual_information_rate(system, [0, 1], [2, 3]),
        rtol=2e-9,
        atol=2e-10,
    )
    torch.testing.assert_close(
        gaussian_transfer_entropy_rate(transformed, [0, 1], [2, 3]),
        gaussian_transfer_entropy_rate(system, [0, 1], [2, 3]),
        rtol=2e-9,
        atol=2e-10,
    )
...
def test_monte_carlo_ols_estimates_converge_to_theoretical_mir_te_and_gc():
    ...
    truth = torch.stack(
        [
            _as_scalar(gaussian_mutual_information_rate(true_system, [0], [1])),
            _as_scalar(gaussian_transfer_entropy_rate(true_system, [0], [1])),
            _as_scalar(
                state_space_temporal_mvgc(
                    true_system, [0], [1], conditional=()
                )
            ),
        ]
    )
...
def test_float32_tracks_float64_for_information_rates_and_spectral_integrals():
    ...
    torch.testing.assert_close(
        quantities32, quantities64, rtol=2e-4, atol=2e-5
    )
...
def test_cuda_matches_cpu_for_public_faes_primitives():
    ...
    torch.testing.assert_close(
        gaussian_mutual_information_rate(system_cuda, [0], [1]).cpu(),
        gaussian_mutual_information_rate(system_cpu, [0], [1]),
        rtol=1e-9,
        atol=1e-10,
    )
    torch.testing.assert_close(
        gaussian_transfer_entropy_rate(system_cuda, [0], [1]).cpu(),
        gaussian_transfer_entropy_rate(system_cpu, [0], [1]),
        rtol=1e-9,
        atol=1e-10,
    )
    torch.testing.assert_close(
        innovations_spectral_density(system_cuda, frequency_cuda).cpu(),
        innovations_spectral_density(system_cpu, frequency_cpu),
        rtol=1e-9,
        atol=1e-10,
    )
