...
from complextorch.measures import state_space_temporal_mvgc, state_space_spectral_mvgc
...
class Audit:
    ...
    def compare(self, quantity, left_name, left, right_name, right, atol):
        ...
        self.rows.append({"quantity": quantity, "comparison": f"{left_name}_vs_{right_name}", "max_abs_error": error, "tolerance": atol, "passed": passed, "left_shape": list(np.asarray(left).shape), "right_shape": list(np.asarray(right).shape)})
    ...
    def bound(self, quantity, value, upper):
        ...
        self.rows.append({"quantity": quantity, "comparison": "residual_bound", "max_abs_error": value, "tolerance": upper, "passed": passed, "left_shape": [], "right_shape": []})
        print(f"{'PASS' if passed else 'FAIL'} {quantity}: {value:.3e} <= {upper:.3e}")

    def finish(self, output):
        output.mkdir(parents=True, exist_ok=True)
        metadata = {"python": sys.version, "numpy": np.__version__, "torch": torch.__version__, "complextorch_file": ct.__file__, "complexbox_file": __import__("complexbox").__file__, "github_sha": os.environ.get("GITHUB_SHA"), "complexbox_expected_commit": "87b5e2cd9bba22ddd978bade6f614da7d6190db2", "rows": self.rows}
        (output / "installed_repo_triplets.json").write_text(json.dumps(metadata, indent=2), encoding="utf-8")
        classified_rows = [{"suite": "installed_repo_triplets", "check": f"{row['quantity']}::{row['comparison']}", "status": "PASS_NUMERICAL" if row["passed"] else "FAIL", "metric": "max_abs_error", "observed": None, "reference": None, "absolute_error": row["max_abs_error"], "relative_error": None, "tolerance": row["tolerance"], "notes": f"left_shape={row['left_shape']}, right_shape={row['right_shape']}"} for row in self.rows]
        (output / "installed_repo_triplets_layera.json").write_text(json.dumps(classified_rows, indent=2), encoding="utf-8")
...
def main():
    ...
    var_cases = [
        ("var1", np.array([[[0.55, 0.00, 0.00], [0.35, 0.45, 0.00], [0.00, 0.25, 0.35]]]), np.diag([1.0, 0.8, 0.6])),
        ("var3", np.array([[[0.40, 0.15, 0.00], [0.00, 0.35, 0.10], [0.00, 0.00, 0.30]], [[-0.12, 0.00, 0.00], [0.08, -0.10, 0.00], [0.00, 0.05, -0.08]], [[0.05, 0.00, 0.00], [0.00, 0.04, 0.00], [0.03, 0.00, 0.02]]]), np.array([[1.0, 0.2, 0.1], [0.2, 0.8, 0.15], [0.1, 0.15, 0.6]])),
    ]
    ...
    ref_p = solve_discrete_are(a.T, c.T, q, r); ref_p = 0.5 * (ref_p + ref_p.T)
    ref_v = c @ ref_p @ c.T + r
    ...
    ref_pg = solve_discrete_are(a.T, c.T, q, r, s=s); ref_pg = 0.5 * (ref_pg + ref_pg.T)
    ...
    cb_gamma, _ = mvgc.ss_to_autocov(a, c, cb_k, cb_v, qmax=-max_lag); cb_gamma = np.transpose(cb_gamma, (2, 0, 1))
    ...
    conditional_coefficients = np.array([[[0.35, 0.10, 0.00, 0.00], [0.00, 0.30, 0.12, 0.00], [0.08, 0.00, 0.28, 0.10], [0.00, 0.05, 0.00, 0.25]], [[-0.08, 0.00, 0.00, 0.00], [0.03, -0.06, 0.00, 0.00], [0.00, 0.04, -0.05, 0.00], [0.00, 0.00, 0.02, -0.04]]], dtype=float)
    conditional_sigma = np.array([[1.0, 0.25, 0.10, 0.05], [0.25, 0.9, 0.18, 0.08], [0.10, 0.18, 0.8, 0.12], [0.05, 0.08, 0.12, 0.7]], dtype=float)
    conditional_cb_coefficients = np.transpose(conditional_coefficients, (1, 2, 0))
    conditional_ct_system = ct.build_var_system(torch.tensor(conditional_coefficients), torch.tensor(conditional_sigma))
    conditional_cb_a, conditional_cb_c, conditional_cb_k, conditional_cb_v = mvgc.var_to_ss(conditional_cb_coefficients, conditional_sigma)
    ct_temporal_default = np.squeeze(_np(state_space_temporal_mvgc(conditional_ct_system, source=[1], target=[0])))
    ct_temporal_explicit = np.squeeze(_np(state_space_temporal_mvgc(conditional_ct_system, source=[1], target=[0], conditional=[2, 3])))
    cb_temporal = np.asarray(mvgc.ss_to_mvgc(conditional_cb_a, conditional_cb_c, conditional_cb_k, conditional_cb_v, [0], [1]))
    audit.compare("mvgc.default_conditional.temporal.default_vs_explicit_remaining", "complextorch_default", ct_temporal_default, "complextorch_explicit_remaining", ct_temporal_explicit, ATOL_TIGHT)
    audit.compare("mvgc.default_conditional.temporal.ct_vs_complexbox", "complextorch_default", ct_temporal_default, "complexbox_default", cb_temporal, ATOL_TIGHT)
    ct_spectral_default = np.squeeze(_np(state_space_spectral_mvgc(conditional_ct_system, source=[1], target=[0], frequencies=torch.tensor(frequencies))))
    ct_spectral_explicit = np.squeeze(_np(state_space_spectral_mvgc(conditional_ct_system, source=[1], target=[0], conditional=[2, 3], frequencies=torch.tensor(frequencies))))
    cb_spectral = np.asarray(mvgc.ss_to_smvgc(conditional_cb_a, conditional_cb_c, conditional_cb_k, conditional_cb_v, [0], [1], 512))
    audit.compare("mvgc.default_conditional.spectral.default_vs_explicit_remaining", "complextorch_default", ct_spectral_default, "complextorch_explicit_remaining", ct_spectral_explicit, 5e-9)
    audit.compare("mvgc.default_conditional.spectral.ct_vs_complexbox", "complextorch_default", ct_spectral_default, "complexbox_default", cb_spectral, 5e-9)
    audit.finish(output)
