...
consume canonical model objects and derive intermediate covariances,
autocovariances and spectra from those parameters so generating and fitted
models are evaluated through exactly the same code path.
"""
...
from .dynamics import active_information_storage, autocovariances, cross_spectral_density, entropy_rate, predictive_information, spectral_entropy
from .emergence import emergence_measures
from .gaussian import dual_total_correlation, gaussian_entropy, gaussian_mutual_information, o_information, s_information, total_correlation
...
class ModelMeasureConfig:
    """Optional structural choices required by non-scalar model measures."""
...
def stationary_observation_covariance(model: VARSystem | LinearDynamicalSystem) -> torch.Tensor:
    ...
    return model.observation @ model.state_covariance @ model.observation.transpose(-1, -2) + model.observation_covariance


def pairwise_gaussian_mutual_information(covariance: torch.Tensor, *, base: float = 2.0) -> torch.Tensor:
    ...
    result = torch.zeros((*covariance.shape[:-2], n_variables, n_variables), dtype=covariance.dtype, device=covariance.device)
    for left in range(n_variables):
        for right in range(left + 1, n_variables):
            ...
            value = gaussian_mutual_information(covariance.index_select(-2, index).index_select(-1, index), 1, base=base)
            result[..., left, right] = value
            result[..., right, left] = value
    return result


def gaussian_measures_from_model(model: VARSystem | LinearDynamicalSystem, *, base: float = 2.0) -> dict[str, torch.Tensor]:
...
def past_future_covariance(system: VARSystem, variables: tuple[int, int], *, lag: int = 1) -> torch.Tensor:
...
def phiid_from_model(system: VARSystem, variables: tuple[int, int], *, lag: int = 1) -> dict[str, torch.Tensor]:
...
def compute_all_model_measures(model: Model, config: ModelMeasureConfig | None = None) -> dict[str, Any]:
    ...
    are supplied. No quantity is estimated directly from samples. Control
    measures are evaluated only for an explicit ``LinearDynamicalSystem``;
    converting a VAR companion form with zero observation noise is not treated
    as a generic latent-state observation model.
    """
    ...
    if isinstance(model, VARSystem):
        ...
        if config.source is not None and config.target is not None:
            mvgc = {
                "temporal": state_space_temporal_mvgc(model, source=config.source, target=config.target, conditional=config.conditional, base=config.base)
            }
            if config.frequencies is not None:
                mvgc["spectral"] = state_space_spectral_mvgc(model, source=config.source, target=config.target, conditional=config.conditional, frequencies=config.frequencies, base=config.base)
            result["mvgc"] = mvgc
        if config.phiid_variables is not None:
            result["phiid"] = phiid_from_model(model, config.phiid_variables, lag=config.phiid_lag)
        ...
        result["control"] = {"dynamical_dependence": dynamical_dependence(model, base=config.base)}
        if config.partition is not None:
            result["control"]["stochastic_interaction"] = stochastic_interaction(model, config.partition, base=config.base)

    elif isinstance(model, InnovationsStateSpace):
        if config.source is None or config.target is None:
            raise ValueError("source and target are required to evaluate an InnovationsStateSpace")
        mvgc = {
            "temporal": state_space_temporal_mvgc(model, source=config.source, target=config.target, conditional=config.conditional, base=config.base)
        }
        if config.frequencies is not None:
