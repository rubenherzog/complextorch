...
def _grassmann_project_row(projection: torch.Tensor, euclidean: torch.Tensor) -> torch.Tensor:
...
def _riemannian_armijo_single(
    ...
    *,
    ...
    with torch.no_grad():
        for state_index in range(2, max_iterations + 1):
            ...
            for _ in range(max_backtracks):
                ...
                if bool(torch.isfinite(candidate_value) & (candidate_value <= armijo_bound)):
                    accepted = True
                    break
            ...
            if bool(torch.abs(previous - value) <= objective_tolerance * scale):
                convergence = 2
                break
    ...
    return DDRiemannianSearchResult(
        ...
        convergence=torch.tensor([convergence], dtype=torch.int64, device=matrix.device),
        step_size=step_size.reshape(1),
        iterations=torch.tensor([iterations], dtype=torch.int64, device=matrix.device),
        objective_evaluations=torch.tensor(
...
def _riemannian_armijo(
    ...
    *,
    ...
    history_values = None
    if history:
        history_values = torch.cat([run.history for run in runs if run.history is not None])
...
def optimise_dynamical_dependence_proxy_riemannian(
    ...
    *,
    ...
    result = _riemannian_armijo(
        ...
        gradient=lambda matrix: proxy_dynamical_dependence_autograd_gradient(
            matrix, sequence
        ),
        max_iterations=max_iterations,
        initial_step_size=initial_step_size,
...
def optimise_dynamical_dependence_spectral_riemannian(
    ...
    *,
    ...
    result = _riemannian_armijo(
        initial,
        objective=lambda matrix: spectral_dynamical_dependence(matrix, transfer),
        gradient=lambda matrix: spectral_dynamical_dependence_autograd_gradient(
            matrix, transfer
        ),
        max_iterations=max_iterations,
        initial_step_size=initial_step_size,
